...
| t16 Turso 連線 stream 過期自動重連 | 完成 | 驗收回饋：get_connection 以 @st.cache_resource 快取單一 Turso 連線，Hrana stream 閒置隔夜被伺服器端關閉後重用即拋「stream not found」404（本機與 Cloud 長駐皆會發生）。需偵測連線失效自動清快取重連並重試，不讓使用者看到 stream not found |

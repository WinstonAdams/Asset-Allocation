...
### 決策建議
推薦改用 `libsql`，理由：官方唯一現行的遠端連線套件，且原 proposal 寫法已失效。**此項屬 proposal 修正：SPEC §二／proposal §外部依賴 的套件名已更正為 `libsql`（於 1-2a commit caae4df 落定）。**
...
1. **【已完成】Turso 套件名修正**：proposal §外部依賴 與 SPEC §二 的 `libsql-client` 已更正為 `libsql`（時效性硬問題——舊套件已隨 AWS 遷移失效；於 1-2a commit caae4df 落定）。
